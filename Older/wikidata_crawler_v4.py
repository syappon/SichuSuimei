"""
Wikidata Famous People Crawler v4
シンプル版 - 名前・生年月日・職業のみ取得

変更点:
- 職業カテゴリ分類を廃止、無作為取得
- クエリを極限まで軽量化（タイムアウト対策）
- 初回リクエストのウォームアップ処理を追加
"""
...
HEADERS = {
    "User-Agent": "ShichusuimeiResearchBot/4.0 (academic research, non-commercial)",
...
def build_query(limit: int, offset: int) -> str:
    """
    可能な限りシンプルなクエリ。
    - SERVICE wikibase:label を排除（最大の重さの原因）
    - ORDER BY を排除
    - 職業絞り込みなし・完全無作為
    - ラベルはQIDから別途取得
    """
    ...
    with open(OUTPUT_NDJSON, ndjson_mode, encoding="utf-8") as ndf:
        while total_records < target:
            ...
            for b in bindings:
                ...
                occupation = fetch_occupation_label(occ_qid, occ_cache)
                time.sleep(1)
...
if __name__ == "__main__":
    ...
    log.info("=" * 55)
    log.info("Wikidata Crawler v4 起動")
